...
async function getLocalResults(dir) {
	const outputFiles = await readdir(dir);
	return await Promise.all(
		outputFiles.map(async item => {
			return {
				name: basename(item, ".json"),
				result: JSON.parse(await readFile(resolve(dir, item)))
			};
		})
	);
}

export async function compare(base, current) {
	...
	const [baseResults, currentResults, buildInfo] = await Promise.all([
		getLocalResults(base),
		getLocalResults(current),
		fetchBuildInfo()
	]);
	const baseData = {};
	const currentData = {};
